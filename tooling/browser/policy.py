...
def browser_human_behavior_enabled(default: bool = False) -> bool:
    raw = os.getenv("CORTEXPILOT_BROWSER_HUMAN_BEHAVIOR", "")
    return _to_bool(raw, default)


def browser_human_behavior_level(default: str = "") -> str:
    raw = os.getenv("CORTEXPILOT_BROWSER_HUMAN_BEHAVIOR_LEVEL")
    if raw is None:
        return default
    stripped = raw.strip()
...
def browser_plugin_optional(default: bool = True) -> bool:
    raw = os.getenv("CORTEXPILOT_BROWSER_PLUGIN_OPTIONAL", "")
    return _to_bool(raw, default)


def env_bool(name: str, default: bool = False) -> bool:
    if name == "CORTEXPILOT_BROWSER_HUMAN_BEHAVIOR":
        return browser_human_behavior_enabled(default=default)
    if name == "CORTEXPILOT_BROWSER_PLUGIN_OPTIONAL":
        return browser_plugin_optional(default=default)
    return default


def env_text(name: str, default: str = "") -> str:
    if name == "CORTEXPILOT_BROWSER_HUMAN_BEHAVIOR_LEVEL":
