...
EXTERNAL_WEB_PROBE="${OPENVIBECODING_CI_EXTERNAL_WEB_PROBE_GATE:-1}"
EXTERNAL_WEB_PROBE_URL="${OPENVIBECODING_EXTERNAL_WEB_PROBE_URL:-https://example.com}"
EXTERNAL_WEB_PROBE_PROVIDER_API_MODE="${OPENVIBECODING_EXTERNAL_WEB_PROBE_PROVIDER_API_MODE:-auto}"
EXTERNAL_WEB_PROBE_PROVIDER_TIMEOUT_SEC="${OPENVIBECODING_EXTERNAL_WEB_PROBE_PROVIDER_TIMEOUT_SEC:-15}"
EXTERNAL_WEB_PROBE_HARD_TIMEOUT_SEC="${OPENVIBECODING_EXTERNAL_WEB_PROBE_HARD_TIMEOUT_SEC:-180}"
PYTHON_BIN="${OPENVIBECODING_UI_REGRESSION_PYTHON:-${OPENVIBECODING_PYTHON:-}}"
...
resolve_external_probe_provider_mode_or_fail() {
  ...
  case "$mode" in
    require|auto|off)
      ;;
    *)
      echo "❌ [ui-ops] invalid OPENVIBECODING_EXTERNAL_WEB_PROBE_PROVIDER_API_MODE=${mode} (allowed: require|auto|off)" >&2
      exit 2
      ;;
  esac
...
if [[ "$EXTERNAL_WEB_PROBE" == "1" ]]; then
  ...
  echo "⚠️ [ui-ops] skip external web probe (OPENVIBECODING_CI_EXTERNAL_WEB_PROBE_GATE=0, break-glass audited)"
fi
...
if [[ "$REQUIRE_FULL_STRICT" == "1" ]]; then
  run_with_heartbeat_and_timeout "ui-ops-full-strict-ui" "${UI_REGRESSION_FULL_STRICT_TIMEOUT_SEC:-7200}" "$HEARTBEAT_SEC" -- \
    env OPENVIBECODING_UI_FULL_E2E_RUN_ID="$full_strict_run_id" npm run ui:e2e:full:gemini:strict
...
OPENVIBECODING_UI_TRUTH_GATE_STRICT="$STRICT_TRUTH" \
OPENVIBECODING_UI_P0_REPORT="$p0_report" \
OPENVIBECODING_UI_P1_REPORT="$p1_report" \
OPENVIBECODING_UI_CLICK_INVENTORY_REPORT="$full_strict_click_report" \
OPENVIBECODING_UI_CLICK_INVENTORY_REQUIRED="$CLICK_INVENTORY_REQUIRED" \
