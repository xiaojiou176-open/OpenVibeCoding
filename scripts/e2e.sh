...
CORTEXPILOT_E2E_NON_SERIAL_PARALLEL="${CORTEXPILOT_E2E_NON_SERIAL_PARALLEL:-1}"
CORTEXPILOT_E2E_NON_SERIAL_WORKERS="${CORTEXPILOT_E2E_NON_SERIAL_WORKERS:-auto}"
CORTEXPILOT_E2E_HEARTBEAT_INTERVAL_SEC="${CORTEXPILOT_E2E_HEARTBEAT_INTERVAL_SEC:-20}"
CORTEXPILOT_E2E_FAST_GATE_TIMEOUT_SEC="${CORTEXPILOT_E2E_FAST_GATE_TIMEOUT_SEC:-900}"
CORTEXPILOT_E2E_LIVE_PREFLIGHT_TIMEOUT_SEC="${CORTEXPILOT_E2E_LIVE_PREFLIGHT_TIMEOUT_SEC:-180}"
CORTEXPILOT_E2E_NON_SERIAL_TIMEOUT_SEC="${CORTEXPILOT_E2E_NON_SERIAL_TIMEOUT_SEC:-2400}"
CORTEXPILOT_E2E_SERIAL_TIMEOUT_SEC="${CORTEXPILOT_E2E_SERIAL_TIMEOUT_SEC:-1800}"
SKIP_FAST_GATE=0
...
if [[ "$SKIP_FAST_GATE" -eq 1 ]]; then
  ...
  run_with_heartbeat_and_timeout "e2e-fast-gate-test-quick" "$CORTEXPILOT_E2E_FAST_GATE_TIMEOUT_SEC" "$CORTEXPILOT_E2E_HEARTBEAT_INTERVAL_SEC" -- \
...
if [[ "$SKIP_LIVE_PREFLIGHT" -eq 1 ]]; then
  ...
  run_with_heartbeat_and_timeout "e2e-live-preflight" "$CORTEXPILOT_E2E_LIVE_PREFLIGHT_TIMEOUT_SEC" "$CORTEXPILOT_E2E_HEARTBEAT_INTERVAL_SEC" -- \
...
export CORTEXPILOT_ENABLE_MCP_E2E="${CORTEXPILOT_ENABLE_MCP_E2E:-1}"
if [[ -z "${CORTEXPILOT_REQUIRE_MCP_E2E:-}" ]]; then
  if [[ -n "${CI:-}" ]] || [[ "${CORTEXPILOT_CI_PROFILE:-}" == "strict" ]]; then
    export CORTEXPILOT_REQUIRE_MCP_E2E="1"
  elif command -v codex >/dev/null 2>&1; then
    export CORTEXPILOT_REQUIRE_MCP_E2E="1"
  else
    export CORTEXPILOT_REQUIRE_MCP_E2E="0"
    echo "⚠️ [e2e] codex not found; default CORTEXPILOT_REQUIRE_MCP_E2E=0 (MCP e2e will skip instead of fail)"
  fi
else
  export CORTEXPILOT_REQUIRE_MCP_E2E
fi
if ! command -v codex >/dev/null 2>&1 && [[ "${CORTEXPILOT_REQUIRE_MCP_E2E}" =~ ^(1|true|yes)$ ]]; then
  export CORTEXPILOT_REQUIRE_MCP_E2E="0"
  echo "⚠️ [e2e] codex not found but CORTEXPILOT_REQUIRE_MCP_E2E was truthy; force-downgrade to 0 for deterministic minimal-runner compatibility"
fi
if [ "$CORTEXPILOT_E2E_NON_SERIAL_PARALLEL" = "1" ]; then
  NON_SERIAL_WORKERS="$CORTEXPILOT_E2E_NON_SERIAL_WORKERS"
...
run_with_heartbeat_and_timeout "e2e-non-serial" "$CORTEXPILOT_E2E_NON_SERIAL_TIMEOUT_SEC" "$CORTEXPILOT_E2E_HEARTBEAT_INTERVAL_SEC" -- \
...
run_with_heartbeat_and_timeout "e2e-serial" "$CORTEXPILOT_E2E_SERIAL_TIMEOUT_SEC" "$CORTEXPILOT_E2E_HEARTBEAT_INTERVAL_SEC" -- \
