...
PROJECT_CONTEXT = """你正在审计 CortexPilot Web Dashboard。

CortexPilot 是一个“AI 管家/指挥塔”系统，核心目标是：
...
PAGE_ANALYSIS_PROMPT = """你是 CortexPilot UI 审计官。请分析单页截图并输出 JSON（不要附加多余文本）。
...
INTERACTION_ANALYSIS_PROMPT = """你是 CortexPilot 交互验收官。你会收到一次按钮交互的前后截图与运行观察数据。请输出 JSON（不要附加多余文本）。
