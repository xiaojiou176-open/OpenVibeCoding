...
MODE="${OPENVIBECODING_DEAD_CODE_NIGHTLY_MODE:-warn}"
if [[ "$MODE" != "warn" && "$MODE" != "gate" ]]; then
  echo "❌ [dead-code-nightly] invalid mode: $MODE (expected warn|gate)"
  exit 2
fi

ARCHIVE_ROOT="${OPENVIBECODING_DEAD_CODE_NIGHTLY_ARCHIVE_ROOT:-.runtime-cache/openvibecoding/reports/dead_code/nightly}"
TREND_JSONL="${OPENVIBECODING_DEAD_CODE_NIGHTLY_TREND_JSONL:-.runtime-cache/openvibecoding/reports/dead_code/nightly_trend.jsonl}"
LATEST_JSON="${OPENVIBECODING_DEAD_CODE_NIGHTLY_LATEST_JSON:-.runtime-cache/openvibecoding/reports/dead_code/latest_full_scan.json}"
LATEST_MD="${OPENVIBECODING_DEAD_CODE_NIGHTLY_LATEST_MD:-.runtime-cache/openvibecoding/reports/dead_code/latest_full_scan.md}"
TREND_WINDOW="${OPENVIBECODING_DEAD_CODE_NIGHTLY_TREND_WINDOW:-7}"
TREND_FAIL_ON_REGRESSION="${OPENVIBECODING_DEAD_CODE_NIGHTLY_TREND_FAIL_ON_REGRESSION:-1}"
TREND_MAX_SYMBOL_DELTA="${OPENVIBECODING_DEAD_CODE_NIGHTLY_TREND_MAX_SYMBOL_DELTA:-0}"
TREND_MAX_LINE_DELTA="${OPENVIBECODING_DEAD_CODE_NIGHTLY_TREND_MAX_LINE_DELTA:-0}"
TREND_CONSECUTIVE_INCREASE_FAIL="${OPENVIBECODING_DEAD_CODE_NIGHTLY_TREND_CONSECUTIVE_INCREASE_FAIL:-3}"
