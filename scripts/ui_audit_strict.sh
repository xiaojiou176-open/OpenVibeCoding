...
STRICT_TIMEOUT_SEC="${OPENVIBECODING_UI_AUDIT_STRICT_TIMEOUT_SEC:-5400}"
STRICT_HEARTBEAT_SEC="${OPENVIBECODING_UI_AUDIT_STRICT_HEARTBEAT_SEC:-30}"
STRICT_ENGINE="${OPENVIBECODING_UI_AUDIT_STRICT_ENGINE:-parallel}"

if [[ "$STRICT_ENGINE" != "parallel" && "$STRICT_ENGINE" != "single" ]]; then
  echo "❌ [ui-audit:strict] invalid OPENVIBECODING_UI_AUDIT_STRICT_ENGINE=$STRICT_ENGINE (expected: parallel|single)" >&2
  exit 2
fi
