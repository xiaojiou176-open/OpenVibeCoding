...
PID_FILE="$ROOT_DIR/.runtime-cache/openvibecoding/temp/openvibecoding-dev-api.pid"
HOST="${OPENVIBECODING_DEV_HOST:-127.0.0.1}"
DASHBOARD_PORT="${OPENVIBECODING_DASHBOARD_PORT:-3100}"
...
stop_port_listener_if_repo_process() {
  ...
  for pid in $pids; do
    ...
    if [[ "$cmd" == *"$ROOT_DIR/apps/dashboard"* || "$cmd" == *"$ROOT_DIR/apps/desktop"* || ( -n "$PYTHON_BIN" && "$cmd" == *"$PYTHON_BIN"* ) || "$cmd" == *"openvibecoding_orch.cli serve"* ]]; then
...
stop_repo_dev_processes_by_pattern "pnpm --dir apps/dashboard dev|openvibecoding-dashboard@0\\.1\\.0 dev" "Dashboard pnpm wrapper"
stop_repo_dev_processes_by_pattern "npm --prefix apps/desktop run tauri:dev|openvibecoding-desktop@0\\.1\\.0 tauri:dev" "Desktop tauri npm wrapper"
...
for pid in $(ps -Ao pid,command | rg "${PYTHON_BIN:-$ROOT_DIR/.runtime-cache/cache/toolchains/python/current/bin/python} -m openvibecoding_orch\\.cli serve --host $HOST --port 19[0-9]{3}" | awk '{print $1}'); do
