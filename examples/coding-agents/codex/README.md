...
This folder shows the narrowest truthful CortexPilot setup for Codex:
...
- `marketplace.example.json`: copy to `.agents/plugins/marketplace.json` when
  you want Codex to discover the local CortexPilot bundle from this repo clone
...
2. Copy `../mcp/readonly.mcp.json.example` to the MCP config path you use for
   Codex and replace `__CORTEXPILOT_REPO_ROOT__`.
