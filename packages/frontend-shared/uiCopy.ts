  ...
  "zh-CN": {
    ...
    dashboard: {
      homePhase2: {
        ...
        productSpineDescription:
          "首页先讲实时指挥台，再把主循环钉在三块真相面上：指挥塔、工作流案例和证明与回放。",
        productSpineCards: [
          {
            href: "/command-tower",
            title: "指挥塔",
        ...
        publicTemplateCards: [
          ...
          },
          {
            ...
            badge: "公开展示",
            title: "topic_brief",
            desc: "把一个有边界的话题简报做成只读工作流案例，并附带搜索证据。",
            bestFor: "适合围绕一个主题和最近时间窗做窄范围简报。",
            example: "Seattle tech and AI + 7d + 5 results",
            proof: "证明状态：公开，但尚未作为发布级基线",
            fields: ["topic", "time_range", "max_results"],
          },
          {
            href: "/pm?template=page_brief",
            badge: "已追踪浏览器证明包",
            title: "page_brief",
            desc: "围绕单个 URL 和浏览器证据生成只读工作流案例。",
            bestFor: "适合单页比整段主题更重要的情况。",
            example: "https://example.com + focused summary request",
            proof: "证明状态：已追踪的浏览器公开证明包",
            fields: ["url", "focus"],
          },
        ],
        ...
        publicAdvantageCards: [
          {
            href: "/pm",
            title: "提示词工程",
            desc: "把任务合同、范围、约束、交付物和验收条件讲清楚，而不是继续给模型塞一坨大提示词。",
          },
          {
            href: "/workflows",
            title: "上下文工程",
          ...
          },
          {
            href: "/contracts",
            title: "护栏工程",
            desc: "把合约、审批、运行时绑定和证明面全部接进轨道与护栏里，让系统能安全续跑，而不是只赌模型发挥。",
          },
        ],
        ...
        integrationDescription:
          "先把指挥台认清，再把兼容性矩阵当成主路由，把“先证明再说”的指南当成最快建立信任的入口，最后再进入协议、作业手册、package 或 AI 页面。",
        ...
        integrationCards: [
          {
            badge: "决策入口",
            title: "兼容性矩阵",
            desc: "如果你想先用一页看懂 Codex、Claude Code、OpenClaw、只读 MCP、技能手册和构建者入口的关系，就从这里开始。",
            href: "/compatibility/",
          },
          {
            badge: "真实采用地图",
          ...
          },
          {
            badge: "团队作业手册",
            title: "技能手册入口",
            desc: "如果你的团队需要可复用的智能体作业手册，而不是自己从 `.agents/skills/` 猜规则，就从这里开始。",
            href: "/skills/",
          },
        ],
        ...
        builderQuickstartCtaLabel: "打开构建者快速入口",
        builderQuickstartCtaHref: "/builders/",
        builderCards: [
          {
            badge: "协议快速入口",
        ...
        liveCaseGalleryDescription:
          "把真实工作流案例当作可验的参考案例来使用。这里会直接链接可实时查看的案例详情和可分享 recap，而不是伪造演示专用的画廊数据。",
        liveCaseGalleryActionLabel: "打开工作流案例",
        liveCaseGalleryActionHref: "/workflows",
        firstTaskGuideTitle: "首个任务指南（可展开）",
        firstTaskGuideDescription:
          "从请求开始，观察指挥塔，确认工作流案例，再检查证明与回放。默认折叠，避免首屏过吵。",
        ...
        firstTaskGuideSteps: [
          ...
          },
          {
            ...
            step: "第 3 步",
            title: "确认工作流案例",
            desc: "打开工作流案例，确认持久案例记录、队列姿态和关联运行。",
          },
          {
            href: "/runs",
            prefetch: true,
        ...
        optionalApprovalStep: {
          ...
          prefetch: true,
          step: "可选",
          title: "审批检查点（仅在需要人工复核时）",
          desc: "用审批台确认被阻塞步骤，并完成最后放行。",
        },
      },
      ...
      approval: {
        pageTitle: "人工审批",
        pageSubtitle: "在继续执行前，先把需要你拍板的事项集中看完。",
        panelTitle: "审批工作台",
        panelIntro:
          "这里先给你看待拍板事项，再把权限缺口和读取失败单独提示。队列安静，不代表全局以后都不再需要人工确认。",
        ...
        recoveryTip: "恢复建议：先确认登录状态、审批权限和后端连通性，再进行重试。",
        ...
        reasonLabel: "卡住原因",
        requiredActionLabel: "需要你确认的点",
        resumeAtLabel: "批准后恢复位置",
        continueButton: "批准并继续",
        continuingButton: "审批中...",
        manualHint:
          "如果某个 run 已经卡在人工确认，但没有出现在待处理卡片里，可以手动粘贴运行 ID 从这里继续处理。",
        runIdLabel: "运行 ID",
        ...
        statusApproved: "已批准，系统会继续执行。",
        statusFailed: (message: string) => `审批失败：${message}`,
      },
      commandTowerPage: {
      ...
      runDetailPage: {
        ...
        proofNextStepFallback: "在提升或分享任何结果前，先检查当前运行报告。",
      },
      workflowDetailPage: {
        title: "工作流案例详情",
        subtitle: "先判断风险，再确认案例摘要、运行映射、队列姿态和事件时间线，然后再做治理动作。",
        ...
        degradedNextAction:
          "现在只能用可见的案例身份、事件时间线和运行映射来做诊断。在数据链路恢复前，不要执行审批、回滚、重放或队列动作。",
        degradedBadge: "只读",
        degradedIdentityTitle: "身份快照（降级）",
        degradedRunMappingTitle: "运行映射样本（降级）",
        degradedRunMappingEmpty: "降级模式下当前没有可验证的运行映射。",
        degradedRunMappingReadonlyNote: "只读说明：这里只能拿运行链路做判断，不能直接拿来执行治理动作。",
      ...
      workflowListPage: {
        title: "工作流案例",
        subtitle:
          "工作流案例会把队列姿态、关联运行、结论，以及后续的证明与回放决策都绑在同一条操作记录上。",
      ...
      runsPage: {
        title: "证明与回放",
        subtitle:
          "这里是运行证据、对比姿态和重放决策的主入口。失败分流只是其中一条操作路径，不是这张页面的全部意义。",
        countsBadge: (runCount: number) => `${runCount} 个运行`,
        warningTitle: "证明与回放当前处于部分真相模式。",
        warningNextStep: "先把当前可见运行当成只读快照，再进入对应的运行详情做治理判断。",
        metricLabels: {
          runInventory: "运行库存",
          replayPosture: "重放姿态",
        ...
        operatorPriorityHeadline: (failed: number) => `先复核失败运行：${failed}`,
        operatorPriorityClearHeadline: "当前没有失败运行需要复核",
        operatorPrioritySubline: "优先打开失败通道，再从 Run Detail 看证明、对比和重放。",
        operatorPriorityClearSubline: "失败通道目前是空的。继续观察新运行，或者从 PM 入口发起下一条请求。",
        operatorPrimaryActionFailed: "打开失败证明通道",
        operatorPrimaryActionClear: "启动新任务",
        operatorSecondaryAction: "查看失败事件",
        filterAriaLabel: "证明与回放状态筛选",
        filters: {
          all: "全部运行",
          failed: "失败",
          running: "运行中",
        ...
        firstScreenLimit: (visibleCount: number) =>
          `首屏只显示最新 ${visibleCount} 条运行，避免证明复核面一上来就过载。`,
      },
      contractsPage: {
        title: "合约桌",
        subtitle:
          "把这里当成合约分诊台：先确认执行权、技能/MCP 配置、运行时绑定和护栏，再决定这条任务应不应该继续往前跑。",
      ...
      agentsPage: {
        title: "角色桌",
        subtitle:
          "把这里当成角色治理台：先确认哪条执行线失稳、哪些执行席位真实在线、调度队列卡在哪，再决定回到哪条 run 或 workflow 继续处理。",
        ...
        metricLabels: {
          riskDesk: "风险执行线",
          executionSeats: "在线执行席位",
          schedulerPosture: "调度积压",
        },
        metricBadges: {
          riskActive: "先处理操作风险",
          riskClear: "当前无失败执行线",
          schedulerNeedsAction: "需要调度动作",
          schedulerStable: "可继续观察",
        },
        metricSublines: {
          risk: (statuses: number, healthy: number) => `状态机总数 ${statuses} / 非失败 ${healthy}`,
          riskHint: "这张卡只看运行状态，用来决定先去哪条线做证据复核。",
          execution: (activeAgents: number, capacityRatio: number) => `已绑定代理 ${activeAgents}（绑定率 ${capacityRatio}%）`,
          executionHint: "这张卡只回答“谁真的在线、谁真的绑上了”；它不负责描述等待调度的队列积压。",
          scheduler: (unassignedStatuses: number, unassignedFailedStatuses: number) =>
        ...
        roleCatalog: {
          title: "角色目录（只读镜像）",
          subtitle:
            "这里展示角色目的、技能包、MCP 工具包、运行时绑定和执行权摘要，但目录本身不是执行权来源。",
          fullList: "完整列表",
          registryUnavailable: "角色目录暂时不可用。你仍可继续查看执行线分诊和锁状态。",
        ...
        filters: {
          title: "角色、席位与调度筛选",
          subtitle: "用角色和关键词把已绑定席位、调度积压和失败记录拆开看，避免把不同问题混成一张板。",
          searchPlaceholder: "搜索 run / agent / task / path",
          allRoles: "全部角色",
          applyFilter: "应用筛选",
          hint: "字段留空时就是完整角色桌；只有当调查路径明确后，再应用筛选。",
          clearFilter: "清除筛选",
        },
        stateMachine: {
          title: "执行线分诊",
          subtitle: "按 run 逐条看执行线。这里的“待调度”表示调度积压，不等于代理本身出故障。",
          summaryBadge: (statuses: number, sample: number) => `状态机 ${statuses} 条（首屏样本 ${sample} 条）`,
          failedBadge: (failed: number, currentPageFailed: number) => `失败执行线 ${failed} 条（当前页 ${currentPageFailed} 条）`,
          unassignedFailuresBadge: (count: number) => `未分配失败 ${count} 条`,
          viewFailedRuns: "批量查看失败运行",
          emptyTitle: "当前没有活动运行",
