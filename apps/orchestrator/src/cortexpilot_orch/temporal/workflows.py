...
def _isolated_temporal_env() -> Any:
    snapshot_temporal_activity = os.environ.get("CORTEXPILOT_TEMPORAL_ACTIVITY")
    snapshot_temporal_workflow = os.environ.get("CORTEXPILOT_TEMPORAL_WORKFLOW")
    snapshot_temporal_workflow_id = os.environ.get("CORTEXPILOT_TEMPORAL_WORKFLOW_ID")
    try:
        yield
    finally:
        if snapshot_temporal_activity is None:
            os.environ.pop("CORTEXPILOT_TEMPORAL_ACTIVITY", None)
        else:
            os.environ["CORTEXPILOT_TEMPORAL_ACTIVITY"] = snapshot_temporal_activity
        if snapshot_temporal_workflow is None:
            os.environ.pop("CORTEXPILOT_TEMPORAL_WORKFLOW", None)
        else:
            os.environ["CORTEXPILOT_TEMPORAL_WORKFLOW"] = snapshot_temporal_workflow
        if snapshot_temporal_workflow_id is None:
            os.environ.pop("CORTEXPILOT_TEMPORAL_WORKFLOW_ID", None)
        else:
            os.environ["CORTEXPILOT_TEMPORAL_WORKFLOW_ID"] = snapshot_temporal_workflow_id
...
if activity and workflow:
    ...
    async def run_contract_activity(request: RunRequest) -> dict[str, Any]:
        with _isolated_temporal_env():
            ...
            os.environ["CORTEXPILOT_TEMPORAL_WORKFLOW_ID"] = request.workflow_id
    ...
    @workflow.defn
    class CortexPilotRunWorkflow:
        @workflow.run
        async def run(self, request: RunRequest) -> dict[str, Any]:
            timeout_sec = int(os.getenv("CORTEXPILOT_TEMPORAL_ACTIVITY_TIMEOUT", "3600"))
    ...
    class CortexPilotRunWorkflow:  # type: ignore[no-redef]
        pass
