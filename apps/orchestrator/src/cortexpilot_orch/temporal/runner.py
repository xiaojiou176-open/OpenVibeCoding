...
from cortexpilot_orch.temporal.manager import temporal_required


def _enabled() -> bool:
    return os.getenv("CORTEXPILOT_TEMPORAL_WORKFLOW", "").strip().lower() in {"1", "true", "yes"}


def _config() -> tuple[str, str, str]:
    address = os.getenv("CORTEXPILOT_TEMPORAL_ADDRESS", "localhost:7233")
...
def _workflow_id(task_id: str) -> str:
    ...
    base = task_id or "cortexpilot"
    return f"cortexpilot-{base}-{suffix}"
...
def run_workflow(repo_root: Path, contract_path: Path, mock_mode: bool) -> dict[str, Any]:
    ...
    from cortexpilot_orch.temporal.workflows import CortexPilotRunWorkflow, RunRequest
    ...
    async def _run() -> dict[str, Any]:
        ...
        try:
            handle = await client.start_workflow(
                CortexPilotRunWorkflow.run,  # type: ignore[arg-type]
                request,
                id=workflow_id,
