...
def _enabled() -> bool:
    if os.getenv("CORTEXPILOT_TEMPORAL_ACTIVITY", "").strip() in {"1", "true", "yes"}:
        return False
    return os.getenv("CORTEXPILOT_TEMPORAL_ENABLED", "").strip().lower() in {"1", "true", "yes"}


def _required() -> bool:
    return os.getenv("CORTEXPILOT_TEMPORAL_REQUIRED", "").strip().lower() in {"1", "true", "yes"}
