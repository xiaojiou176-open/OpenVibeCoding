...
def requires_human_approval(
    *,
    ...
    env_flag = os.getenv("CORTEXPILOT_GOD_MODE_REQUIRED", "").strip().lower()
    if env_flag in {"1", "true", "yes"}:
        return True

    on_request_flag = os.getenv("CORTEXPILOT_GOD_MODE_ON_REQUEST", "").strip().lower()
...
def force_unlock_requested() -> bool:
    raw = os.getenv("CORTEXPILOT_FORCE_UNLOCK", "").strip().lower()
    return raw in {"1", "true", "yes"}


def auto_lock_cleanup_requested() -> bool:
    raw = os.getenv("CORTEXPILOT_LOCK_AUTO_CLEANUP", "").strip().lower()
    if raw in {"1", "true", "yes"}:
        return True
    ttl_raw = os.getenv("CORTEXPILOT_LOCK_TTL_SEC", "").strip()
    if ttl_raw != "":
        return True
    default_raw = os.getenv("CORTEXPILOT_LOCK_TTL_SEC_DEFAULT", "").strip()
    return default_raw != ""


def god_mode_timeout_sec() -> int:
    timeout_raw = os.getenv("CORTEXPILOT_GOD_MODE_TIMEOUT_SEC", "0").strip()
    try:
