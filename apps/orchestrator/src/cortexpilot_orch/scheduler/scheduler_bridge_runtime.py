...
from cortexpilot_orch.scheduler import core_helpers, rollback_pipeline, tool_execution_pipeline
...
def _build_runner_via_execution_adapter(
    ...
    if runner_name not in _ADAPTER_RUNTIME_RUNNERS:
        return None
    try:
        execution_adapter = import_module("cortexpilot_orch.runners.execution_adapter")
...
def select_runner(contract: dict[str, Any], store: RunStore) -> object:
    ...
    runner_name = _normalize_runner_name(runtime_runner, _normalize_runner_name(os.getenv("CORTEXPILOT_RUNNER"), "agents"))
