...
def maybe_execute_temporal_workflow(
    *,
    ...
    temporal_workflow = os.getenv("CORTEXPILOT_TEMPORAL_WORKFLOW", "").strip().lower() in {"1", "true", "yes"}
    in_temporal_activity = os.getenv("CORTEXPILOT_TEMPORAL_ACTIVITY", "").strip().lower() in {"1", "true", "yes"}
