...
from cortexpilot_orch.scheduler.execute_task_preflight import (
    maybe_execute_temporal_workflow,
    prepare_runtime_and_policy_gates,
)
from cortexpilot_orch.scheduler.execute_task_types import ExecutionRuntimeState, ExecutionSetup


@contextmanager
def isolated_execution_env() -> Any:
    snapshot_cortexpilot_run_id = os.environ.get("CORTEXPILOT_RUN_ID")
    snapshot_cortexpilot_trace_id = os.environ.get("CORTEXPILOT_TRACE_ID")
    snapshot_cortexpilot_codex_profile = os.environ.get("CORTEXPILOT_CODEX_PROFILE")
    snapshot_cortexpilot_codex_version = os.environ.get("CORTEXPILOT_CODEX_VERSION")
    snapshot_cortexpilot_network_approved = os.environ.get("CORTEXPILOT_NETWORK_APPROVED")
    snapshot_codex_home = os.environ.get("CODEX_HOME")
    try:
        yield
    finally:
        if snapshot_cortexpilot_run_id is None:
            os.environ.pop("CORTEXPILOT_RUN_ID", None)
        else:
            os.environ["CORTEXPILOT_RUN_ID"] = snapshot_cortexpilot_run_id
        if snapshot_cortexpilot_trace_id is None:
            os.environ.pop("CORTEXPILOT_TRACE_ID", None)
        else:
            os.environ["CORTEXPILOT_TRACE_ID"] = snapshot_cortexpilot_trace_id
        if snapshot_cortexpilot_codex_profile is None:
            os.environ.pop("CORTEXPILOT_CODEX_PROFILE", None)
        else:
            os.environ["CORTEXPILOT_CODEX_PROFILE"] = snapshot_cortexpilot_codex_profile
        if snapshot_cortexpilot_codex_version is None:
            os.environ.pop("CORTEXPILOT_CODEX_VERSION", None)
        else:
            os.environ["CORTEXPILOT_CODEX_VERSION"] = snapshot_cortexpilot_codex_version
        if snapshot_cortexpilot_network_approved is None:
            os.environ.pop("CORTEXPILOT_NETWORK_APPROVED", None)
        else:
            os.environ["CORTEXPILOT_NETWORK_APPROVED"] = snapshot_cortexpilot_network_approved
...
def prepare_execution_setup(
    *,
    ...
    runner_name = runtime_runner or os.getenv("CORTEXPILOT_RUNNER", "agents").strip().lower()
    allow_codex_exec = allow_codex_exec_fn()
    parent_task_id = str(contract.get("parent_task_id", "")).strip()
    diagnostic_root_override = os.getenv("CORTEXPILOT_DIAGNOSTIC_RUNS_ROOT", "").strip()
    runs_root_override = os.getenv("CORTEXPILOT_RUNS_ROOT", "").strip()
    diagnostic_mode = (
        runner_name == "codex"
    ...
    if diagnostic_mode:
        raw_root = diagnostic_root_override or ".runtime-cache/cortexpilot/runs_diagnostic"
        diagnostic_root = Path(raw_root) if raw_root else Path(".runtime-cache/cortexpilot/runs_diagnostic")
    ...
    workflow_id = os.getenv("CORTEXPILOT_TEMPORAL_WORKFLOW_ID", "").strip()
    workflow_info: dict[str, Any] | None = None
    if workflow_id:
        workflow_info = {
            "workflow_id": workflow_id,
            "task_queue": os.getenv("CORTEXPILOT_TEMPORAL_TASK_QUEUE", "cortexpilot-orch"),
            "namespace": os.getenv("CORTEXPILOT_TEMPORAL_NAMESPACE", "default"),
            "status": "RUNNING",
        }
    ...
    manifest: dict[str, Any] = {
        ...
        "versions": {
            "contracts_schema": "v1",
            "orchestrator": os.getenv("CORTEXPILOT_ORCHESTRATOR_VERSION", "local"),
            "codex_cli": codex_version or "unknown",
            "python": sys.version.split()[0],
            "node": os.getenv("CORTEXPILOT_NODE_VERSION", ""),
        },
        "tasks": [],
        "artifacts": [],
        "evidence_hashes": {},
