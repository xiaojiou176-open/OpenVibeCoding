...
from cortexpilot_orch.runners import agents_mcp_config, agents_prompting, mcp_streaming as _mcp_streaming
...
from cortexpilot_orch.runners.agents_mcp_execution_helpers import run_worker_execution
from cortexpilot_orch.runners.agents_mcp_runtime_helpers import (
    MCPMessageArchive,
    bind_mcp_log_paths,
)
...
def runtime_root_from_store(store: RunStore) -> Path:
    ...
    return Path(os.getenv("CORTEXPILOT_RUNTIME_ROOT", str(fallback)))
...
def materialize_worker_codex_home(
    ...
    *,
    ...
    catalog_home_raw = os.getenv("CORTEXPILOT_CODEX_BASE_HOME", "").strip()
    if catalog_home_raw:
    ...
    if missing:
        raise RuntimeError("mcp_tool_set missing in base config " f"(set CORTEXPILOT_CODEX_BASE_HOME): {missing}")
...
def patch_mcp_codex_event_notifications() -> tuple[bool, str]:
    ...
    if getattr(mcp_types, "_CORTEXPILOT_CODEX_EVENT_PATCHED", False):
    ...
    try:
        ...
        mcp_types._CORTEXPILOT_CODEX_EVENT_PATCHED = True
...
def patch_mcp_initialized_notification() -> tuple[bool, str]:
    ...
    if getattr(mcp_session, "_CORTEXPILOT_INITIALIZED_PATCHED", False):
        return True, "already patched"
    ...
    mcp_session._CORTEXPILOT_INITIALIZED_PATCHED = True
    mcp_session._CORTEXPILOT_INITIALIZED_ORIGINAL = original_initialize
    return True, "patched"
