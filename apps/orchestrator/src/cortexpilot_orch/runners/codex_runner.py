...
from cortexpilot_orch.observability.codex_event_parser import parse_codex_event_line
...
def _use_output_schema() -> bool:
    raw = os.getenv("CORTEXPILOT_CODEX_USE_OUTPUT_SCHEMA", "").strip().lower()
    return raw in {"1", "true", "yes", "on"}


def _exec_timeout_sec() -> int:
    raw = os.getenv("CORTEXPILOT_CODEX_EXEC_TIMEOUT_SEC", "300").strip()
    try:
        value = int(raw)
    except ValueError:
        value = 300
...
class CodexRunner:
    ...
    def run_contract(
        ...
        allow_codex = os.getenv("CORTEXPILOT_ALLOW_CODEX_EXEC", "").strip().lower() in {"1", "true", "yes"}
            ...
            self._store.append_event(
                run_id,
                {
                    ...
                    "meta": {
                        ...
                        "env": "CORTEXPILOT_CODEX_USE_OUTPUT_SCHEMA",
                    },
                },
            )
        if model:
        ...
        codex_version = os.getenv("CORTEXPILOT_CODEX_VERSION", "").strip() or None
