...
from cortexpilot_orch.runners.agents_runner_result_helpers import finalize_agents_run_result
    ...
    *,
    ...
    try:
        llm_timeout_sec = _resolve_positive_float(
            os.getenv("CORTEXPILOT_AGENTS_LLM_TIMEOUT_SEC", "180.0").strip(),
            180.0,
        )
        llm_client_retries = _resolve_non_negative_int(
            os.getenv("CORTEXPILOT_AGENTS_LLM_CLIENT_RETRIES", "2").strip(),
            2,
        )
    ...
    async def _run_streamed(
        ...
        extra_headers = {
            "x-cortexpilot-run-id": run_id,
            "x-cortexpilot-task-id": task_id,
        }
        ...
        fallback_enabled = os.getenv("CORTEXPILOT_AGENTS_STREAM_TIMEOUT_FALLBACK", "").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
        ...
        stream_timeout_retries = _resolve_non_negative_int(
            os.getenv("CORTEXPILOT_AGENTS_STREAM_TIMEOUT_RETRIES", "2").strip(),
            2,
        )
        retry_backoff_sec = _resolve_positive_float(
            os.getenv("CORTEXPILOT_AGENTS_STREAM_RETRY_BACKOFF_SEC", "1.5").strip(),
            1.5,
        )
