...
from cortexpilot_orch.runners import agents_events, agents_payload, mcp_server_lifecycle, mcp_streaming
from cortexpilot_orch.runners.agents_stream_watch_helpers import (
    StreamWatchState,
    cancel_watch_tasks,
    start_watch_tasks,
)
...
async def consume_stream_events(
    *,
    ...
    idle_timeout_raw = os.getenv("CORTEXPILOT_STREAM_IDLE_TIMEOUT_SEC", "").strip()
    ...
    hard_timebox_raw = os.getenv("CORTEXPILOT_CODEX_TIMEBOX_SEC", "").strip()
    ...
    if hard_timebox_sec is not None and hard_timebox_sec <= 0:
        hard_timebox_sec = None
    broken_pipe_enabled = os.getenv("CORTEXPILOT_MCP_BROKEN_PIPE_FAIL", "1").strip().lower() in {
        "1",
        "true",
        "yes",
    }
