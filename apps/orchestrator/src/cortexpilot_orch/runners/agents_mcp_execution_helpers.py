...
async def run_worker_execution(
    *,
    ...
    class _CortexPilotMCPServerStdio(mcp_server_stdio_cls):
        def __init__(
            self,
            params: Any,
            *,
    ...
    env["CORTEXPILOT_PROVIDER"] = provider
    codex_base_url = resolve_codex_base_url()
    if codex_base_url:
        env["CORTEXPILOT_PROVIDER_BASE_URL"] = codex_base_url
    ...
    if resolved_api_key:
        if normalized_provider == "gemini":
            ...
            env.setdefault("CORTEXPILOT_EQUILIBRIUM_API_KEY", resolved_api_key)
    server = _CortexPilotMCPServerStdio(
