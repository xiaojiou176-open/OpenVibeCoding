...
from cortexpilot_orch.chain import chain_lifecycle, chain_reporting
from cortexpilot_orch.chain.helpers import _check_exclusive_paths, _normalize_depends, _step_task_id
from cortexpilot_orch.chain.runner_execution_helpers import (
    execute_chain_step as _execute_chain_step,
    execute_task_subprocess as _execute_task_subprocess_helper,
)
from cortexpilot_orch.chain.runtime_helpers import (
    _SCHEMA_KEYS_CACHE,
...
class ChainRunner:
    ...
    def _chain_exec_mode(self) -> str:
        return os.getenv("CORTEXPILOT_CHAIN_EXEC_MODE", "inline").strip().lower()

    def _resolve_chain_subprocess_timeout_sec(self) -> float | None:
        raw = os.getenv("CORTEXPILOT_CHAIN_SUBPROCESS_TIMEOUT_SEC", "").strip()
        if not raw:
            return None
        try:
            value = float(raw)
