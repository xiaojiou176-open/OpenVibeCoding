...
def _resolve_running_stale_sec() -> int:
    raw = os.getenv("CORTEXPILOT_RUNNING_LOCK_STALE_SEC", "").strip()
    if raw:
        try:
...
def _force_unlock_enabled() -> bool:
    return _truthy(os.getenv("CORTEXPILOT_FORCE_UNLOCK", ""))


def _owner_token() -> str:
    explicit = os.getenv("CORTEXPILOT_LOCK_OWNER_TOKEN", "").strip()
    if explicit:
        return explicit
    run_id = os.getenv("CORTEXPILOT_RUN_ID", "").strip()
    if run_id:
...
def acquire_lock(allowed_paths: Iterable[str], reclaim_stale: bool = True) -> bool:
    ...
    run_id = os.getenv("CORTEXPILOT_RUN_ID", "unknown")
    paths = _sanitize_allowed_paths(allowed_paths)
    if not paths:
        return False
