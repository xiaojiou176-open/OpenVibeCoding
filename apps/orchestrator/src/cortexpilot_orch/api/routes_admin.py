...
from cortexpilot_orch.api.security_validators import validate_run_id
from cortexpilot_orch.config import load_config
from cortexpilot_orch.services.orchestration_service import OrchestrationService, sanitize_approval_payload
...
def _request_role(request: Request | None) -> str:
    if request is None:
        return ""
    return request.headers.get("x-cortexpilot-role", "").strip().upper()
...
def _role_header_is_trusted(request: Request | None) -> bool:
    ...
    if not load_config().api_auth_required:
        return True
    return bool(getattr(request.state, "cortexpilot_api_auth_verified", False))
