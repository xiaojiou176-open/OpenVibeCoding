...
def load_coverage_targets(
    ...
    threshold: float,
    max_workers: int,
    include_prefix: str = "apps/orchestrator/src/cortexpilot_orch/",
...
def _worker_timeout_sec() -> int:
    raw = os.getenv("CORTEXPILOT_COVERAGE_WORKER_TIMEOUT_SEC", "300").strip()
    try:
        value = int(raw)
    except ValueError:
        value = 300
...
def _default_shell_permission() -> str:
    runner_name = os.getenv("CORTEXPILOT_RUNNER", "").strip().lower()
...
def _worker_contract(
    ...
    payload = {
        ...
        "inputs": {
            "spec": (
                "You are the CortexPilot coverage self-heal worker. "
...
def build_coverage_self_heal_chain(
    ...
    resolved_chain_id = chain_id or f"task_chain_cortexpilot_self_heal_coverage_{_now_tag()}"
