...
def validate_mcp_concurrency(mode: str | None) -> dict:
    ...
    if raw in proxy_modes:
        enabled = os.getenv("CORTEXPILOT_MCP_PROXY_ENABLED", "").strip().lower() in {"1", "true", "yes"}
        return {
            "ok": enabled,
            "mode": "proxy",
            "reason": "proxy enabled" if enabled else "proxy disabled; set CORTEXPILOT_MCP_PROXY_ENABLED=1",
        }
    if raw in multi_modes:
        enabled = os.getenv("CORTEXPILOT_MCP_ALLOW_MULTI", "").strip().lower() in {"1", "true", "yes"}
        return {
            "ok": enabled,
            "mode": "multi",
