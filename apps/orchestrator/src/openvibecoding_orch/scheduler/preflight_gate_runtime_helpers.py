...
from openvibecoding_orch.scheduler import gate_orchestration
from openvibecoding_orch.scheduler.preflight_gate_types import BuildResultFn, PreflightOps, PreflightState
...
def run_preflight_pipeline(
    *,
    ...
    if not state.integrated_gate["ok"]:
        ...
        rebuild_policy_gate()
        return finish(False)
    mcp_mode = os.getenv("OPENVIBECODING_MCP_CONCURRENCY_MODE", "single")
    ...
    if not mcp_concurrency.get("ok") and os.getenv("OPENVIBECODING_MCP_CONCURRENCY_REQUIRED", "").strip().lower() in {
        "1",
        "true",
        "yes",
    }:
    ...
    if state.human_approval_required:
        ...
        if approved and requires_network and network_policy == "on-request":
            os.environ["OPENVIBECODING_NETWORK_APPROVED"] = "1"
