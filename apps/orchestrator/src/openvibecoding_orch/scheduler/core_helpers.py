...
def trace_url(trace_id: str, run_id: str) -> str:
    template = os.getenv("OPENVIBECODING_TRACE_URL_TEMPLATE", "").strip()
    if template:
        try:
            return template.format(trace_id=trace_id, run_id=run_id)
        except KeyError:
            return ""
    base = os.getenv("OPENVIBECODING_TRACE_BASE_URL", "").strip()
    if not base:
        return ""
...
def per_run_codex_home_enabled() -> bool:
    raw = os.getenv("OPENVIBECODING_CODEX_HOME_PER_RUN", "").strip().lower()
