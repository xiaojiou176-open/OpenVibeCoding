...
from openvibecoding_orch.runners.provider_resolution import ProviderCredentials, resolve_preferred_api_key
...
def enable_lock_auto_cleanup_for_coverage() -> dict[str, Any]:
    lock_auto_cleanup_raw = os.getenv("OPENVIBECODING_LOCK_AUTO_CLEANUP", "").strip().lower()
    if lock_auto_cleanup_raw in {"1", "true", "yes"}:
        cleanup_source = "env"
    else:
        os.environ["OPENVIBECODING_LOCK_AUTO_CLEANUP"] = "1"
    ...
    if ttl_raw:
        ...
        os.environ["OPENVIBECODING_LOCK_TTL_SEC"] = "120"
...
def enable_chain_subprocess_timeout_for_coverage() -> dict[str, Any]:
    chain_mode_raw = os.getenv("OPENVIBECODING_CHAIN_EXEC_MODE", "").strip().lower()
    if chain_mode_raw:
        chain_mode = chain_mode_raw
        chain_mode_source = "env"
    else:
        os.environ["OPENVIBECODING_CHAIN_EXEC_MODE"] = "subprocess"
        chain_mode = "subprocess"
        chain_mode_source = "coverage_execute_default"

    timeout_raw = os.getenv("OPENVIBECODING_CHAIN_SUBPROCESS_TIMEOUT_SEC", "").strip()
    if timeout_raw:
        timeout_source = "env"
        ...
        os.environ["OPENVIBECODING_CHAIN_SUBPROCESS_TIMEOUT_SEC"] = "300"
...
def ensure_coverage_python_env(repo_root: Path) -> dict[str, Any]:
    raw = os.getenv("OPENVIBECODING_PYTHON", "").strip()
    if raw:
    ...
    if candidate.exists() and os.access(candidate, os.X_OK):
        value = str(candidate)
        os.environ["OPENVIBECODING_PYTHON"] = value
...
def prepare_coverage_execute_env(
    ...
    *,
    ...
    if mock_mode:
        return {
            "mode": "mock",
            "runner": os.getenv("OPENVIBECODING_RUNNER", "agents") or "agents",
    ...
    runner_env_raw = os.getenv("OPENVIBECODING_RUNNER", "").strip().lower()
    ...
    if fallback_base_url and equilibrium_healthcheck_fn(fallback_base_url):
        os.environ["OPENVIBECODING_PROVIDER_BASE_URL"] = fallback_base_url
        os.environ.setdefault("OPENVIBECODING_EQUILIBRIUM_API_KEY", "local-equilibrium")
        return {
    ...
    if not runner_env_raw:
        ...
        codex_allow_exists = bool(os.getenv("OPENVIBECODING_ALLOW_CODEX_EXEC", "").strip())
