...
import cortexpilot_orch.cli_coverage_helpers as coverage_helpers
...
def test_enable_lock_cleanup_and_chain_timeout_env_paths(monkeypatch) -> None:
    ...
    monkeypatch.setenv("CORTEXPILOT_LOCK_TTL_SEC", "abc")
    ...
    monkeypatch.setenv("CORTEXPILOT_CHAIN_EXEC_MODE", "THREAD")
    monkeypatch.setenv("CORTEXPILOT_CHAIN_SUBPROCESS_TIMEOUT_SEC", "12.7")
...
def test_ensure_python_env_and_prepare_execute_env_modes(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("CORTEXPILOT_PYTHON", "/tmp/custom-python")
