from cortexpilot_orch.temporal.manager import notify_run_started, notify_run_completed


def test_temporal_notify_skipped(monkeypatch) -> None:
    monkeypatch.delenv("CORTEXPILOT_TEMPORAL_ENABLED", raising=False)
...
def test_temporal_notify_missing_lib(monkeypatch) -> None:
    monkeypatch.setenv("CORTEXPILOT_TEMPORAL_ENABLED", "1")
...
def test_temporal_manager_helpers(monkeypatch) -> None:
    monkeypatch.setenv("CORTEXPILOT_TEMPORAL_ACTIVITY", "1")
    monkeypatch.setenv("CORTEXPILOT_TEMPORAL_ENABLED", "1")
    ...
    monkeypatch.setenv("CORTEXPILOT_TEMPORAL_NAMESPACE", "cortexpilot")
    ...
    assert started["namespace"] == "cortexpilot"
