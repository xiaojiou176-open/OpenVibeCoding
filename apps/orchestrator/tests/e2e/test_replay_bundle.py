...
def test_replay_verify_bundle(tmp_path: Path) -> None:
    store = RunStore(runs_root=tmp_path)
    run_id = store.create_run("task_replay")
    ...
    contract = {
        ...
        "inputs": {"spec": "verify replay bundle", "artifacts": _output_schema_artifacts("worker")},
    ...
    review_report = _review_report(run_id, "task_replay")
    test_report = _test_report(run_id, "task_replay", ["echo", "ok"])
