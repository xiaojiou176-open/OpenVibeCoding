# OpenVibeCoding Distribution Contract

This file is the canonical human-readable source for OpenVibeCoding's current
...
| `@openvibecoding/frontend-shared` | `not standalone distribution unit` | Repo-owned presentation substrate for dashboard / desktop / future web surfaces | repo-local package only | frontend presentation helpers only | keep repo-owned for now |
...
- Implementation: `apps/orchestrator/src/openvibecoding_orch/mcp_readonly_server.py`
- Canonical startup wrapper: `scripts/run_openvibecoding_readonly_mcp.sh`
